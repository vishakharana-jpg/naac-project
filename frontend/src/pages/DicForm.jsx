...
const metrics = [
  ...
  },
  {
    ...
    subheading:
      "Better conflict resolution: Mechanisms available. Appropriate weightage for availability of an effective Grievance Redressal Mechanism and resolution of complaints.",
    type: "checkbox",
    options: [
  ...
  },
  {
    ...
    subheading:
      "The university has implemented the policy on Digital India and its e-governance activities are reflected through",
    type: "checkbox",
    options: [
      "NAD Implementation",
      "ABC Implementation",
...
const tableFieldLabels = {
  ...
  ews: "EWS",
  pwdMale: "PwD Male",
  pwdFemale: "PwD Female",
  pwdState: "PwD State/Other",
};
  ...
  const handleSubmit = async () => {
    try {
      ...
      const metricsData = metrics.map((metric) => ({
        ...
        checked: metric.options.filter((opt) => checked[`${metric.id}-${opt}`]),
      ...
      <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border border-slate-100">
            ...
            <div className="bg-slate-50 px-5 py-3 border-b border-slate-200 flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-800">
                  Student Enrollment (DSB Campus / Bhimtal Campus)
                </h3>
                <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                  Total number of students admitted in the first year of various
                  programmes, year-wise, during the last two years.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr>
                    <th
                      rowSpan={2}
                      className="border border-slate-200 bg-slate-50 px-2 py-2 text-slate-600 font-medium text-center w-8"
                    >
                      S.No
                    </th>
                    <th
                      rowSpan={2}
                      className="border border-slate-200 bg-slate-50 px-2 py-2 text-slate-600 font-medium text-left min-w-[130px]"
                    >
                      Programme Name
                    </th>
                    <th
                      rowSpan={2}
                      className="border border-slate-200 bg-slate-50 px-2 py-2 text-slate-600 font-medium text-center"
                    >
                      Applications Received
                    </th>
                    <th
                      rowSpan={2}
                      className="border border-slate-200 bg-slate-50 px-2 py-2 text-slate-600 font-medium text-center min-w-[80px]"
                    >
                      Admitted LSC (Total)
                    </th>
                    <th
                      colSpan={4}
                      className="border border-slate-200 bg-blue-50 px-2 py-2 text-blue-700 font-medium text-center"
                    >
                      Category
                    </th>
                    <th
                      colSpan={3}
                      className="border border-slate-200 bg-indigo-50 px-2 py-2 text-indigo-700 font-medium text-center"
                    >
                      PwD
                    </th>
                  </tr>
                  <tr>
                    {["SC", "ST", "OBC", "EWS"].map((h) => (
                      <th
                        key={h}
                        className="border border-slate-200 bg-blue-50 px-2 py-1.5 text-blue-600 font-medium text-center"
                      >
                        {h}
                      </th>
                    ))}
                    {["Male", "Female", "State / Other State"].map((h) => (
                      <th
                        key={h}
                        className="border border-slate-200 bg-indigo-50 px-2 py-1.5 text-indigo-600 font-medium text-center"
                      >
                        {h}
                      </th>
                    ))}
                    ...
                    <tr key={row.id} className="hover:bg-slate-50">
                      ...
                      <td className="border border-slate-200 px-1 py-1">
                        <input
                          ...
                          className="w-full text-sm px-1 py-1.5 rounded border border-transparent focus:border-blue-300 focus:bg-blue-50 focus:outline-none bg-transparent"
                        />
                      </td>
                      {tableFields.map((field) => (
                        ...
                        >
                          <input
                            ...
                            value={row[field]}
                            onChange={(e) =>
                              handleTableChange(row.id, field, e.target.value)
                            }
              ...
              <div className="px-5 py-4 space-y-4">
                  ...
                  {metric.options.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`text-sm leading-snug ${isChecked ? "text-blue-800 font-medium" : "text-slate-600"}`}
                        >
                          {option}
                        </span>
                      </label>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleDescription(metric.id, e.target.value)
                    }
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-xs font-medium truncate ${fileNames[metric.id] ? "text-green-700" : "text-slate-500"}`}
                      >
                    ...
                    {fileNames[metric.id] && (
                      <button
                        onClick={(e) => {
                          ...
                          if (fileRefs.current[metric.id])
                            fileRefs.current[metric.id].value = "";
                        }}
        ...
        <div className="sticky bottom-0 bg-white border-t border-slate-100 px-6 py-4 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                exportToExcel(checked, descriptions, fileNames, tableData)
              }
